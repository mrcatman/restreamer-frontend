    ...
    <div class="container">
      ...
      <v-row>
        <v-col>
          ...
          <v-card>
           <v-list-item  @click="joinGame(item)" v-if="!loading && canJoinGame(item)" :key="'game_'+$index" v-for="(item, $index) in games">
              <v-list-item-content>
...
export default {
    ...
    methods: {
      canJoinGame(game) {
          if (game.isMember) {
            return game.state !== "STATE_GAME_WIN" && game.state !== "STATE_GAME_OVER";
          } else {
            return game.state === "STATE_NOT_STARTED";
          }
      },
      joinGame(game) {
        if (game.isMember) {
